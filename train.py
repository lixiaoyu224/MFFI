...
def loss_func():
    ...
    emo_loss_func = nn.CrossEntropyLoss()
...
def train_model(isr_model, data_name, dataloaders, isr_optimizer, scheduler, isr_loss, class_name):
    epochs = 50
    logger = get_logger('/root/lxy/lxy/lxy/ML-ISR/log_new/ResNet152_net2_Multi_16_3.log')

    logger.info('start training!')

    classess = ['amusement', 'anger', 'awe', 'contentment', 'disgust', 'excitement', 'fear', 'sadness']
    # map_8to2={0:0,1:1,2:0,3:0,4:1,5:0,6:1,7:1}
        ...
        for phase in ['train', 'test']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    emo_predicts_1, emo_predicts_2 = isr_model(inputs)
            ...
            if phase == 'test' and epoch_acc > best_acc:
                ...
                }, model_save_path + 'FI' + '_train_best_final_ResNet152_net2_Multi_16_3.pth')

            end_time = datetime.now()  # 获取当前系统时间
            epoch_time = calc_epoch_time(start_time, end_time)
            logger.info('%5s_loss:  %.5f   %.5f  %.5f  lr: %.12f  accuracy:  %.3f%%   %.3f%%   %.3f%%  %5s_time: %s' % (phase,epoch_loss_1,epoch_loss_2, epoch_loss, emo_lr,epoch_acc_1* 100,epoch_acc_2* 100, epoch_acc * 100, phase, epoch_time))
